...
class PlayerLandscapeMetrics {
  ...
  factory PlayerLandscapeMetrics.of(Size size, EdgeInsets safe) {
    ...
    final double artSize = math.max(
      0,
      math.min(
        math.min(paneHeight - 2 * verticalPadding, availableWidth * _artWidthFraction),
        _maxArtSize,
      ),
    );
    ...
    return PlayerLandscapeMetrics._(
      ...
      artRect: Rect.fromLTWH(groupLeft, paneTop + (paneHeight - artSize) / 2, artSize, artSize),
        ...
        Positioned.fromRect(
          ...
          child: Center(
            child: FittedBox(
              ...
              child: SizedBox(width: metrics.controlsRect.width, child: controls),
            ),
          ),
        ),
      ],
