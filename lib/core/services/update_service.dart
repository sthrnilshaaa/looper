...
class UpdateService {
      ...
      switch (result?['status']) {
        ...
        case 'notAllowed' || 'failed':
          ...
          await _announce(
            ...
            message: currentL10n().updateAvailableOnPlay,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (tagName.isNotEmpty && _isUpdateAvailable(currentVersion, tagName)) {
          await _announce(
            key: tagName,
            message: currentL10n().updateAvailableOnGithub(tagName),
            url: htmlUrl,
          );
        }
      }
  ...
  static Future<void> _announce({
    ...
    if (Platform.isAndroid) {
      ...
      final posted = await _channel.invokeMethod<bool>(
        ...
        {'title': currentL10n().updateAvailable, 'body': message, 'url': url},
      );
      if (posted == true) {
    ...
    _showSnackbar(
      title: currentL10n().updateAvailableTitle,
      message: message,
      actionLabel: currentL10n().visit,
      onAction: () async {
        try {
  ...
  static void _showRestartSnackbar() {
    ...
    final controller = _showSnackbar(
      title: currentL10n().updateDownloaded,
      message: currentL10n().restartToInstallUpdate,
      actionLabel: currentL10n().restart,
